...
const VoiceAgentModal = ({
    ...
    const SILENCE_DURATION = 1000; // 1 second of silence to auto-stop
    const MIN_RECORDING_TIME = 800; // Minimum recording time before silence detection kicks in
    const MAX_IDLE_TIMEOUT = 30000; // 30 seconds max idle before auto-close
    ...
    // Reset idle timeout whenever there's activity
    const resetIdleTimeout = useCallback(() => {
        ...
        idleTimeoutRef.current = setTimeout(() => {
            console.log('⏰ Max idle timeout reached (30s), auto-closing...');
    ...
    const startListening = async () => {
        ...
        if (!isModalOpenRef.current) {
            console.log('🚫 Modal closed, skipping startListening');
            return;
        }
    ...
    const stopListeningAndProcess = async () => {
        ...
        try {
            ...
            if (data.audio_base64) {
                ...
                setVoiceState('idle');
                setCurrentCaption('');
                autoRestartTimeoutRef.current = setTimeout(() => startListening(), 1000);
            }
                ...
                audio.onended = () => {
                    ...
                    if (isLastChunk) {
                        setTimeout(() => {
                            ...
                            autoRestartTimeoutRef.current = setTimeout(() => startListening(), 1500);
                            resolve();
                        }, 1000);
                    } else {
                        resolve();
    ...
    const toggleListening = () => {
        if (voiceState === 'speaking') {
            stopAudio();
            autoRestartTimeoutRef.current = setTimeout(() => startListening(), 100);
            return;
        }
    ...
    const handleClose = () => {
        // Mark modal as closed immediately to prevent any new operations
