...
import DelegatedTasksSection from '../components/developer/DelegatedTasksSection';
...
const DeveloperDashboard = () => {
    ...
    const [delegatedTasksLoading, setDelegatedTasksLoading] = useState(true);
    ...
    const fetchDelegatedTasks = async () => {
        try {
            ...
            const response = await api.get('/tasks/my-delegated-tasks');
            if (response.data.success) {
                setDelegatedTasks(response.data.data || []);
            }
        } catch (error) {
    ...
    const handleDelegationComplete = () => {
        ...
        if (user.role === 'developer' || user.role === 'team-lead') {
            fetchDelegatedTasks(); // Refresh delegated tasks list
        }
    ...
    const getFilteredTasks = () => {
        ...
        // Hide completed tasks by default unless explicitly showing them
        if (!showCompleted) {
        ...
        <DashboardLayout sidebarActions={sidebarActions} showBackButtonOverride={false}>
            <div className="min-h-screen bg-black">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    {mentees.length > 0 && (
                        <div className="bg-zinc-900/50 backdrop-blur-md rounded-2xl p-6 mb-8 border border-white/10">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {mentees.map((mentee) => (
                                    <div key={mentee._id} className="bg-zinc-800/50 border border-white/5 rounded-xl p-4 hover:border-cyan-500/30 transition-all">
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <div>
                                                <p className="text-white font-medium">{mentee.firstName} {mentee.lastName}</p>
                                                <p className="text-xs text-zinc-400">{mentee.designation} • {mentee.specializations?.join(', ')}</p>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                    ...
                    {/* My Task Bunches Section */}
                    {bunches.length > 0 && (
                        <div className="bg-zinc-900/50 backdrop-blur-md rounded-2xl p-6 mb-8 border border-white/10">
                            <div className="flex items-center space-x-3 mb-6">
                                <Package className="h-6 w-6 text-emerald-400" />
                                <h2 className="text-xl font-bold text-white">My Task Bunches</h2>
                                <span className="px-3 py-1 bg-emerald-500/10 text-emerald-300 rounded-full text-sm font-semibold">
                                    {bunches.length}
                                </span>
                            </div>
                            {bunchesLoading ? (
                                            ...
                                            <div className="mt-3 space-y-1.5">
                                                {task.assignedBy && (
                                                    <div className="text-xs text-zinc-400 flex items-center gap-1.5">
                                                        ...
                                                        <span>Assigned by: <span className="text-zinc-300">{task.assignedBy.firstName} {task.assignedBy.lastName}</span></span>
                                                        {task.assignedBy.role === 'manager' && <span className="px-1.5 py-0.5 bg-purple-500/20 text-purple-300 rounded text-xs">Manager</span>}
                                                    </div>
                                                )}
                                                {task.delegatedBy && (
