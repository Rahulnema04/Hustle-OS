...
const ManagerDashboard = () => {
  ...
  const [currentView, setCurrentView] = useState('dashboard'); // 'dashboard' or 'leave'
      ...
      try {
        const response = await fetch('http://localhost:8000/api/score-task', {
          ...
          body: JSON.stringify({
            title,
            description,
            phase: phase || newTask.taskBunch || 'Backend Development',
            dependencies: [],
            position: 'mid',
    ...
    return (
      <DashboardLayout sidebarActions={sidebarActions} onBack={handleBackToMain} showBackButtonOverride={true}>
        <ProjectStatus onViewBunches={(projectId) => {
          ...
          setCurrentView('bunch-management');
        }} />
      </DashboardLayout>
    );
  }

  if (currentView === 'bunch-management') {
    return (
      <DashboardLayout sidebarActions={sidebarActions} onBack={() => {
        setCurrentView('project-status');
        setViewingBunchesForProject(null);
      }} showBackButtonOverride={true}>
        ...
        <div className="space-y-6">
          ...
          <div className="dashboard-card">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-lg font-semibold text-gray-900">Projects & Tasks</h2>
              <button
          ...
          {/* Unassigned Automated Tasks Section */}
          {unassignedTasks.length > 0 && (
            <div className="dashboard-card border-l-4 border-purple-500">
              <div className="flex justify-between items-center mb-6">
                <div className="flex items-center space-x-3">
                  <Bot className="h-6 w-6 text-purple-600" />
                  <div>
                    <h2 className="text-lg font-semibold text-gray-900">AI Generated Tasks</h2>
                    <p className="text-sm text-gray-500">These tasks need to be assigned to team members</p>
                  </div>
                </div>
                <div className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-purple-100 text-purple-800">
                  {unassignedTasks.length} tasks awaiting assignment
                </div>
              </div>
          ...
          {reviewTasks.length > 0 && (
            <div className="dashboard-card">
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h2 className="text-lg font-semibold text-gray-900">Tasks Pending Review</h2>
                  <p className="text-sm text-gray-600">Tasks submitted by your team waiting for your approval</p>
                </div>
                <div className="flex items-center text-sm text-gray-500">
                  <Clock className="h-4 w-4 mr-1" />
                  {reviewTasks.length} task{reviewTasks.length !== 1 ? 's' : ''} pending
                </div>
              </div>
          ...
          {/* Task Bunches Section */}
          <div className="dashboard-card">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h2 className="text-lg font-semibold text-gray-900">Task Bunches</h2>
                <p className="text-sm text-gray-600 mt-1">AI-generated task bunches with microtasks - assign entire bunches to team members</p>
              </div>
              <button
                      ...
                      <div className="mb-4">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            // Feature 1: Predict phase as user types
