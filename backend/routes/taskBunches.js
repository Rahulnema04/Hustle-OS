...
router.get('/project/:projectId', authenticateToken, async (req, res) => {
  try {
    const bunches = await TaskBunch.getBunchesByProject(req.params.projectId);
...
// @route   POST /api/task-bunches/:id/unassign
// @access  Private (Manager only)
router.post('/:id/unassign', authenticateToken, authorizeRoles('manager', 'co-founder', 'ceo'), async (req, res) => {
  try {
    const bunch = await TaskBunch.findById(req.params.id).populate({
      path: 'tasks',
      populate: {
