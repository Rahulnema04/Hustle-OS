  ...
  try {
    ...
    switch (req.user.role) {
      ...
      case 'manager':
        ...
        // Filter tasks by these projects OR tasks directly assigned to manager
        // (cofounder-initiated tasks may be assigned directly to manager)
      ...
      case 'intern':
      case 'team-lead':
        // Individuals, developers, interns, and team leads can only see tasks assigned to them
...
const getTaskDetails = async (req, res) => {
  try {
    ...
    const allowedRoles = ['manager', 'ceo', 'hr'];
    
    if (!allowedRoles.includes(userRole)) {
      return res.status(403).json({
        success: false,
        message: 'Access denied. Only managers, CEOs, and HR can view task details.'
      });
    }
...
const managerCompleteTask = async (req, res) => {
  try {
    ...
    if (req.user.role !== 'manager' && req.user.role !== 'ceo' && req.user.role !== 'hr') {
      return res.status(403).json({
        success: false,
...
const requestRevision = async (req, res) => {
  try {
    ...
    // Only allow revision requests for tasks in review status
    if (task.status !== 'review') {
      return res.status(400).json({
        success: false,
        message: 'Only tasks in review status can have revisions requested'
      });
    }
...
const getTasksForReview = async (req, res) => {
  try {
    // Only managers can see tasks for review
    if (req.user.role !== 'manager' && req.user.role !== 'ceo' && req.user.role !== 'hr') {
      return res.status(403).json({
        success: false,
        message: 'Access denied'
    ...
    // Find tasks that are in review status
    ...
    // If user is a manager, filter tasks for their projects only
    let filteredTasks = reviewTasks;
    if (req.user.role === 'manager') {
      filteredTasks = reviewTasks.filter(task => 
        task.project && task.project.assignedManager && 
        task.project.assignedManager.toString() === req.user.id
      );
    }
...
const delegateTask = async (req, res) => {
  try {
    ...
    if (task.assignedTo._id.toString() !== req.user.id) {
      return res.status(403).json({
        success: false,
        message: 'You can only delegate tasks assigned to you'
      });
    }
    ...
    // Fetch current user's full details to get mentorFor
    const currentUser = await User.findById(req.user.id);
    if (!currentUser.mentorFor || currentUser.mentorFor.length === 0) {
      return res.status(400).json({
        success: false,
        message: 'You have no mentees to delegate to'
      });
    }

    // Verify delegate target is in user's mentorFor
    const isMentee = currentUser.mentorFor.some(
    ...
    if (!isMentee) {
      return res.status(403).json({
        success: false,
        message: 'You can only delegate to your mentees'
      });
    }
